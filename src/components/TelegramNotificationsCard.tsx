...
export function TelegramNotificationsCard() {
  ...
  async function runTest() {
    ...
    if (error) setTestStatus(`Error: ${error.message}`);
    else setTestStatus(`Result: ${JSON.stringify(data)}`);
    qc.invalidateQueries({ queryKey: ["notification_events_recent"] });
  }

  if (isLoading || !settings) {
    return <Card title="Telegram Notifications"><p className="text-sm text-muted-foreground">Loading…</p></Card>;
  }
